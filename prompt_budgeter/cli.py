"""
Command-Line Interface for PromptBudgeter: Dynamic Token Budgeting & Key-Value Cache Compressor.
"""
import argparse
import csv
import json
...
from .agents import PromptBudgetCoordinator

coordinator = PromptBudgetCoordinator()


def main(argv=None):
    parser = argparse.ArgumentParser(prog="dynamic-prompt-budget-allocator", description="PromptBudgeter: Dynamic Token Budgeting & Key-Value Cache Compressor")
    ...
    if args.command == "audit":
        ...
        print("=" * 80)
        print(f"  PROMPTBUDGETER: DYNAMIC TOKEN BUDGETING & KEY-VALUE CACHE COMPRESSOR")
        print(f"  Domain: Autonomous Context Management & State Engines | Standard: Token Economy & KV-Cache Management")
    ...
    if args.command == "serve":
        try:
            ...
            app = create_app()
            if app:
                print(f"Starting PromptBudgeter: Dynamic Token Budgeting & Key-Value Cache Compressor on http://{args.host}:{args.port}")
