"""
Pydantic v2 schemas and data definitions for Dynamic Prompt Budget Allocator.
...
class SystemTaskPayload(BaseModel):
    task_id: str = Field(..., description="Unique task / case identifier", min_length=1, max_length=256)
    target_identifier: str = Field(..., description="Entity, patient key, or genomic/cryptographic target", min_length=1, max_length=256)
...
class ConsensusDossier(BaseModel):
    dossier_id: str
    system_slug: str = "dynamic-prompt-budget-allocator"
